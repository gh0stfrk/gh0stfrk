<h1 align="center">Hi, I'm Salman 👋</h1>
<p align="center">
  Software Developer • Backend Engineer • Cloud Enthusiast
</p>

<p align="center">
  <a href="https://salmansyyd.com">🌐 Portfolio</a>
</p>

---

## 👨‍💻 About Me

I build scalable backend systems, cloud-native applications, and reliable data integrations.

Currently at **Travelex**, working on:
- ETL data pipelines
- Enterprise AWS Lambda-based integrations

---

## 💼 Experience

**Sept 2025 – Present**  
**Junior Software Developer — Travelex**

Working on backend modernization initiatives, building and maintaining APIs and integration workflows for high-volume financial systems using Node.js, TypeScript, AWS, and distributed service patterns.

---

## 🛠️ Tech Stack

- **Languages:** Python, JavaScript, Go, SQL
- **Backend:** Node.js, FastAPI, Flask, Django
- **Databases:** PostgreSQL, MySQL, DynamoDB, MSSQL
- **Cloud & DevOps:** AWS (Lambda, S3, DynamoDB, API Gateway, CircleCI), Docker, Terraform
- **Tools:** GitHub, Git, JIRA, Fuse.js, Scrapy

---

## 📈 GitHub Insights

<p align="center">
  <a href="https://github.com/gh0stfrk">
    <img height="170" src="https://github-readme-stats.vercel.app/api?username=gh0stfrk&show_icons=true&theme=radical&hide_border=true&cache_seconds=1800" alt="GitHub Stats" />
  </a>
  <a href="https://github.com/gh0stfrk?tab=repositories">
    <img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=gh0stfrk&layout=compact&theme=radical&hide_border=true&cache_seconds=1800" alt="Top Languages" />
  </a>
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com?user=gh0stfrk&theme=radical&hide_border=true" alt="GitHub Streak" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Focused%20On-Backend%20Engineering-EA4AAA?style=for-the-badge" alt="Focused On Backend Engineering" />
  <img src="https://img.shields.io/badge/Cloud-AWS-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white" alt="Cloud AWS" />
  <img src="https://img.shields.io/badge/Data-ETL%20Pipelines-6C63FF?style=for-the-badge" alt="Data ETL Pipelines" />
</p>

---

## ✨ Current Focus

- Designing robust backend services
- Building efficient data workflows
- Sharpening AWS and cloud architecture skills

---

<p align="center"><i>Code. Learn. Build. Repeat.</i></p>
